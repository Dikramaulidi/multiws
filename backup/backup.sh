...
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/Dikramaulidi/permission/main/ipmini | grep $MYIP | awk '{print $2}')
...
LLatest=`date`
Get_Data () {
git clone https://github.com/Dikramaulidi/backupuseryokko.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "marengkimchi@gmail.com" &> /dev/null
    git config --global user.name "Dikramaulidi" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
    git commit -m backup &> /dev/null
    git branch -M main &> /dev/null
    git remote add origin https://github.com/Dikramaulidi/backupuseryokko
    git push -f https://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@github.com/Dikramaulidi/backupuseryokko.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/Dikramaulidi/backupuseryokko/main/$NameUser/$NameUser.zip"
sleep 1
